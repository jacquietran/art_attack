...
# Create data: Circles 1 -------------------------------------------------------

#generate a seed for our circle with 100000 points and a radius of 50
#the `split_mod` parameter controls how many gaps the circle will have, a higher split_mod gives fewer splits
circle_seed1 <- gen_seed_cir(n_grains = 100000, wind_angle = 270, r = 50, split_mod = 2500)

#generate a list of 5 circles, each one slightly larger than the previous
circles1 <- list(a = circle(100000, r = 50),
...
circles_1 <- 
  circles1 %>%
  map( ~ splitter2(., seed = circle_seed1, wind_angle = 270)) %>%
...
circle_dust_1 <- gust(circles_1, angle = 270, force = 4, diff_mod = 0.006, inertia_mod = 0.0009, jitter_min = 2, jitter_max = 20, jitter_mod = 0.4)

# Create data: Circles 2 -------------------------------------------------------

#generate a seed for our circle with 100000 points and a radius of 30
#the `split_mod` parameter controls how many gaps the circle will have, a higher split_mod gives fewer splits
circle_seed2 <- gen_seed_cir(n_grains = 100000, wind_angle = 270, r = 30, split_mod = 1500)

#generate a list of 3 circles, each one slightly larger than the previous
circles2 <- list(a = circle(100000, r = 30),
                 b = circle(100000, r = 30.25),
                 c = circle(100000, r = 30.5)
)

#split circles and fray the edges
circles_2 <- 
  circles2 %>%
  map( ~ splitter2(., seed = circle_seed2, wind_angle = 270)) %>%
  map_dfr( ~ fray(., min_fray = 100, max_fray = 200, num_fray = 5))

#scatter points according to force and angle
#diff_mod, inertia_mod, and jitter_mod control the shape and look of the points scatter
circle_dust_2 <- gust(circles_2, angle = 270, force = 4, diff_mod = 0.006, inertia_mod = 0.0009, jitter_min = 2, jitter_max = 20, jitter_mod = 0.4)

# Bump x and y values to offset circle position (cf. circle 1)
circle_dust_2 %>%
  mutate(
    x = x + 35,
    y = y - 35) -> circle_dust_2

# Create data: Bind circles ----------------------------------------------------

all_circles <- rbind(circle_dust_1, circle_dust_2)

# Build plot -------------------------------------------------------------------

#plot the points with a low alpha and size
ggplot(all_circles) +
  geom_point(aes(x = x, y = y), alpha = 0.04, size = 0.1, color = "#f7faff", shape = 46) +
  scale_color_identity() +
  theme_void() +
  theme(plot.background = element_rect(fill = "#00318c", colour = NA)) +
  coord_equal()

ggsave("sunprint_circle_test_202101021806.png", height = 10, width = 10, bg = "#00318c")