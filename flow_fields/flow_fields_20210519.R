...
sim <- create_ring(10000) %>%
  ...
  wield(field_force, angle = field, vel = 0.12, xlim = c(-50, 45), ylim = c(-50, 45)) %>%
  evolve(100, record)
...
xmin <- min(traces2$x) / 3
xmax <- max(traces2$x) / 3
ymin <- min(traces2$y) / 3
ymax <- max(traces2$y) / 3
